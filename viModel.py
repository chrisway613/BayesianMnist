    ...
    def evalAllLosses(self) :
        ...
        for m in self.children() :
            if isinstance(m, VIModule) :
                ...
                t_loss = t_loss + m.evalAllLosses() * m.lossScaleFactor
...
class MeanFieldGaussianFeedForward(VIModule):
    ...
    def __init__(
        ...
        self.weights_mean = Parameter(
              (0. if initMeanZero else 1.) * (torch.rand(out_features, int(in_features / groups)) - 0.5)
        )
        self.lweights_sigma = Parameter(
              torch.log(initPriorSigmaScale * weightPriorSigma * torch.ones(out_features, int(in_features / groups)))
        )

        # \mathcal N(0, 1)
        self.noiseSourceWeights = Normal(
              torch.zeros(out_features, int(in_features / groups)),
        ...
        if self.has_bias:
            self.bias_mean = Parameter(
                   (0. if initBiasMeanZero else 1.) * (torch.rand(out_features) - 0.5)
              )
            self.lbias_sigma = Parameter(
                   torch.log(initPriorSigmaScale * biasPriorSigma * torch.ones(out_features))
            )

            # \mathcal N(0, 1)
            self.noiseSourceBias = Normal(
...
class MeanFieldGaussian2DConvolution(VIModule) :
    ...
    def __init__(
        ...
        self.weights_mean = Parameter(
              (0. if initMeanZero else 1.) * (torch.rand(out_channels, int(in_channels / groups), self.kernel_size[0], self.kernel_size[1]) - 0.5)
        )
        self.lweights_sigma = Parameter(
              torch.log(initPriorSigmaScale * wPriorSigma * torch.ones(out_channels, int(in_channels / groups), self.kernel_size[0], self.kernel_size[1]))
        )

        self.noiseSourceWeights = Normal(
              torch.zeros(out_channels, int(in_channels / groups), self.kernel_size[0], self.kernel_size[1]),
        ...
        if self.has_bias:
            ...
            self.addLoss(lambda s: -self.out_channels / 2 * np.log(2*np.pi) - 0.5 * s.samples['bNoiseState'].pow(2).sum() - s.lbias_sigma.sum())
    ...
    def forward(self, x, stochastic=True):
        ...
        if self.padding != 0 and self.padding != (0, 0):
            padkernel = (self.padding, self.padding, self.padding, self.padding) if isinstance(self.padding, int) \
                   else (self.padding[1], self.padding[1], self.padding[0], self.padding[0])
    ...
    def forward(self, x, stochastic=True):
        x = nn.functional.relu(
              nn.functional.max_pool2d(
                self.conv1(x, stochastic=stochastic), 2
            )
        )
        ...
        ''' FC->ReLU->FC->LogSoftmax '''
        x = x.view(-1, 512)
        x = nn.functional.relu(
              self.linear1(x, stochastic=stochastic)
        )
        ...
        # The reason use log-softmax here is for the training loss, which is NLL loss.
